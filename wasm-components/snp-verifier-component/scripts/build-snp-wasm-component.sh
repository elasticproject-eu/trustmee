...
DOCKERFILE_PATH="${DOCKERFILE_PATH:-$ROOT_DIR/wasm-components/snp-verifier-component/docker/Dockerfile.snp-wasm}"
...
docker run --rm \
  ...
  bash -c '
    ...
    rm -rf "$BUILD_ROOT"
    mkdir -p "$BUILD_ROOT"
    cp -a /work/wasm-components/snp-verifier-component "$BUILD_ROOT/snp-verifier-component"
...
EOF
    ...
    # Avoid using /work/rust-toolchain.toml (1.85.1), which is too old for
    # some transient dependencies selected for this isolated workspace.
    cd "$BUILD_ROOT"
