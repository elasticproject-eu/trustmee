...
`cargo build --manifest-path wasm-components/Cargo.toml -p tdx-verifier-component --release --target wasm32-wasip2`
...
For full `attestation-service` API flow (register component + attestation request) with both TDX and SNP:

- `wasm-components/MANUAL_TEST_EXAMPLE.md`
